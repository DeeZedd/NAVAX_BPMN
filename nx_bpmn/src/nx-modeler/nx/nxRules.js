import {
   ...
   */
  export default function nxRules(eventBus) {
    RuleProvider.call(this, eventBus);
  }
  
  inherits(nxRules, RuleProvider);
  
  nxRules.$inject = [ 'eventBus' ];
  
  
  nxRules.prototype.init = function() {
    ...
    function canConnect(source, target) {
      ...
      // only judge about custom elements
      if (!isNavax(source) && !isNavax(target)) {
        return;
      }
      ...
      if (isNavax(source)) {
        if (is(target, 'bpmn:Task')) {
          return { type: 'nx:connection' };
        } else {
          return false;
        }
      } else if (isNavax(target)) {
    ...
    this.addRule('elements.move', HIGH_PRIORITY, function(context) {
      ...
      var allowed = reduce(shapes, function(result, s) {
        ...
        if (type !== isNavax(s) || result === false) {
          return false;
        }
